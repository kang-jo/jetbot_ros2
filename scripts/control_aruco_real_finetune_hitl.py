...
class QTablePolicy:
    ...
    def save(self, path: str) -> Tuple[bool, str]:
        try:
            if not path:
                return False, "empty save path"
            os.makedirs(os.path.dirname(path), exist_ok=True)
            pd.DataFrame(self.Q).to_csv(path, index=False, header=False)
            return True, "OK"
        except Exception as exc:
            return False, f"Failed saving Q-table: {exc}"

    def apply_human_label(
        self,
        state_idx: int,
        action: int,
        positive_q: float = 8.0,
        negative_q: float = -1.0,
        mode: str = "set",
    ) -> None:
        ...
        for a in range(self.n_actions):
            ...
            elif abs(float(self.Q[state_idx, a])) < 1e-12:
                self.Q[state_idx, a] = float(negative_q)
...
class RealControlNodeUdpOnly(Node):
    def __init__(self):
        ...
        self.declare_parameter("resume_from_save_q_table", True)
        ...
        self.declare_parameter("aruco_near_area_ratio", 0.006)
        ...
        self.declare_parameter("hitl_update_mode", "set")  # set / add
        self.declare_parameter("hitl_enforce_safety", True)
        self.declare_parameter("hitl_execute_action", True)
        ...
        if not self.save_q_table_path:
            root, ext = os.path.splitext(self.q_table_path)
            self.save_q_table_path = root + "_real_hitl" + (ext or ".csv")
        self.resume_from_save_q_table = bool(self.get_parameter("resume_from_save_q_table").value)
        self.load_q_table_path = self.q_table_path
        if self.resume_from_save_q_table and os.path.exists(self.save_q_table_path):
            self.load_q_table_path = self.save_q_table_path
        ...
        self.front_zero_as_max = bool(self.get_parameter("front_zero_as_max").value)
        self.front_zero_fallback_m = float(self.get_parameter("front_zero_fallback_m").value)
        self.front_zero_fallback_m = float(max(min(self.front_zero_fallback_m, MAX_SONAR), MIN_SONAR))
        self.front_zero_raw_threshold = float(self.get_parameter("front_zero_raw_threshold").value)
        ...
        self.hitl_on_unknown = bool(self.get_parameter("hitl_on_unknown").value)
        self.hitl_positive_q = float(self.get_parameter("hitl_positive_q").value)
        self.hitl_negative_q = float(self.get_parameter("hitl_negative_q").value)
        self.hitl_update_mode = str(self.get_parameter("hitl_update_mode").value).strip().lower()
        if self.hitl_update_mode not in ("set", "add"):
            self.get_logger().warning(f"hitl_update_mode={self.hitl_update_mode} invalid; fallback to set")
            self.hitl_update_mode = "set"
        self.hitl_enforce_safety = bool(self.get_parameter("hitl_enforce_safety").value)
        self.hitl_execute_action = bool(self.get_parameter("hitl_execute_action").value)
        ...
        loaded, load_msg = self.policy.load(self.load_q_table_path)
        ...
        if loaded:
            ...
            self.get_logger().warning(
                f"Loaded Q-table OK: {self.load_q_table_path} | shape={self.policy.Q.shape} | "
        ...
        self.pending_hitl_state_idx: Optional[int] = None
        self.pending_hitl_sonar_state: Optional[Tuple[int, int, int]] = None
        self.pending_hitl_camera_state: Optional[int] = None
        self.pending_hitl_qrow: Optional[np.ndarray] = None
        self.hitl_count = 0
        self.last_hitl_prompt_time = 0.0
        ...
        self.get_logger().warning(f"Q base path={self.q_table_path}")
        self.get_logger().warning(f"Q load path={self.load_q_table_path}")
        self.get_logger().warning(f"Q save path={self.save_q_table_path}")
        ...
        self.get_logger().warning(
            ...
            f"hitl_on_unknown={self.hitl_on_unknown}"
        )
        self.get_logger().warning(
        ...
        self.get_logger().warning(
            f"front_zero_as_max={self.front_zero_as_max}, "
            f"front_zero_fallback_m={self.front_zero_fallback_m:.2f}, "
            f"front_zero_raw_threshold={self.front_zero_raw_threshold:.3f}"
        )
    ...
    def _set_sonar(self, key: str, value: float) -> None:
        ...
        if used_front_zero_fallback:
            ...
            if now - last > 2.0:
                self.get_logger().warning(
                    f"Front sonar raw={raw_input:.3f} treated as out-of-range -> "
                    f"front={self.raw_sonar[key]:.2f} m"
                )
                self._last_front_zero_fallback_log = now
    ...
    # HITL helpers
    # =============================
    def hitl_input_loop(self) -> None:
        """Read terminal input in background. Commands are processed by control_loop."""
        while not self.hitl_thread_stop.is_set():
            try:
                line = sys.stdin.readline()
                if line == "":
                    time.sleep(0.1)
                    continue
                self.hitl_queue.put(line.strip().lower())
            except Exception:
                time.sleep(0.2)

    @staticmethod
    def human_key_to_action(cmd: str) -> Optional[int]:
        mapping = {
            "e": 0,        # maju
            "a": 1,        # kiri besar
            "s": 3,        # kiri dikit
    ...
    def print_hitl_prompt(
        ...
        now = time.time()
        if not force and now - self.last_hitl_prompt_time < 1.5:
            return
        self.last_hitl_prompt_time = now

        self.get_logger().warning(
            "\n"
            "================ HITL UNKNOWN STATE ================\n"
            f"Robot STOP. State ini belum punya Q-value.\n"
            f"sonar5=(front,left1,right1)={sonar_state} | "
            ...
            f"camera={camera_state} {CAMERA_STATE_NAMES.get(camera_state)} | idx={state_idx} | Q={qrow.tolist()}\n"
            "Input human lalu ENTER:\n"
            "  e = maju\n"
            "  a = kiri besar\n"
            "  s = kiri dikit\n"
            "  d = kanan dikit\n"
            "  f = kanan besar\n"
            "  x = stop/skip tanpa update\n"
            "  q = stop permanen\n"
            "===================================================="
        )
    ...
    def request_hitl(
        ...
        stop(self.vel_pub)
        if not self.awaiting_human or self.pending_hitl_state_idx != state_idx:
            self.awaiting_human = True
            self.pending_hitl_state_idx = int(state_idx)
            self.pending_hitl_sonar_state = tuple(sonar_state)
            self.pending_hitl_camera_state = int(camera_state)
            self.pending_hitl_qrow = qrow.copy()
            self.print_hitl_prompt(sonar_state, camera_state, state_idx, qrow, force=True)
        else:
            self.print_hitl_prompt(sonar_state, camera_state, state_idx, qrow, force=False)

    def clear_hitl_pending(self) -> None:
        self.awaiting_human = False
        self.pending_hitl_state_idx = None
        self.pending_hitl_sonar_state = None
        self.pending_hitl_camera_state = None
        self.pending_hitl_qrow = None

    def process_hitl_if_ready(self) -> bool:
        """
        Return True if this control tick is fully handled by HITL waiting/processing.
        Return False if no HITL is pending.
        """
        ...
        if cmd in ("q", "quit", "exit"):
            self.stop_once("HITL command q -> permanent stop")
            self.clear_hitl_pending()
            return True

        if cmd in ("x", "stop", "skip", " "):
            self.get_logger().warning("HITL skip/stop: no Q update, robot remains stopped for this tick")
            self.clear_hitl_pending()
            return True

        action = self.human_key_to_action(cmd)
        if action is None:
            self.get_logger().warning(
                f"Invalid HITL command '{cmd}'. Use e/a/s/d/f, x, or q."
            )
            return True

        state_idx = self.pending_hitl_state_idx
        sonar_state = self.pending_hitl_sonar_state
        camera_state = self.pending_hitl_camera_state
        if state_idx is None or sonar_state is None or camera_state is None:
            self.get_logger().warning("HITL internal pending state missing; ignoring input")
            self.clear_hitl_pending()
            return True

        if self.hitl_enforce_safety and not self.action_is_safe(action, sonar_state):
            self.get_logger().warning(
                f"HITL action rejected by safety filter: action={action} {ACTION_NAMES.get(action)} "
                f"sonar5={sonar_state}. Pilih aksi lain."
            )
            self.print_hitl_prompt(sonar_state, camera_state, state_idx, self.policy.qrow(state_idx), force=True)
            return True

        old_q = self.policy.qrow(state_idx).copy()
        self.policy.apply_human_label(
            state_idx=state_idx,
            action=action,
            positive_q=self.hitl_positive_q,
            negative_q=self.hitl_negative_q,
            mode=self.hitl_update_mode,
        )
        new_q = self.policy.qrow(state_idx).copy()
        saved, msg = self.policy.save(self.save_q_table_path)
        self.hitl_count += 1

        self.get_logger().warning(
            f"HITL learned: idx={state_idx} sonar5={sonar_state} "
            f"cam={camera_state} {CAMERA_STATE_NAMES.get(camera_state)} "
            f"human_action={action} {ACTION_NAMES.get(action)} | "
            f"Q_old={[round(float(x), 3) for x in old_q.tolist()]} -> "
            f"Q_new={[round(float(x), 3) for x in new_q.tolist()]} | "
            f"saved={saved} ({msg}) | hitl_count={self.hitl_count}"
        )

        self.clear_hitl_pending()

        if self.hitl_execute_action:
